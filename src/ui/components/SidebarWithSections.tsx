  ...
  return (
    ...
    >
      ...
      {children ? (
        <span
          className={SubframeUtils.twClassNames(
            "text-body-bold font-body-bold line-clamp-1 shrink-0 grow basis-0 text-neutral-600",
            { "text-brand-700": selected },
          )}
        >
          {children}
    ...
    return (
      ...
      >
        <div className="flex w-full flex-col items-start gap-4 px-3 py-1">
          {label ? (
            <span className="text-caption-bold font-caption-bold text-subtext-color w-full">
              {label}
            </span>
          ) : null}
        </div>
...
>(function SidebarWithSectionsRoot(
  ...
  return (
    <nav
      className={SubframeUtils.twClassNames(
        "border-neutral-border bg-default-background flex h-full w-60 flex-col items-start border-r border-solid",
        className,
      )}
      ref={ref}
      {...otherProps}
    >
      ...
      {footer ? (
        <div className="border-neutral-border flex w-full items-center gap-4 border-t border-solid px-6 py-6">
          {footer}
        </div>
      ) : null}
    </nav>
