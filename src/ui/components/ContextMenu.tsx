    ...
    return (
      <SubframeCore.ContextMenu.Item asChild={true} {...otherProps}>
        ...
        >
          ...
          {children ? (
            <span className="text-body font-body text-default-font group-hover/9358ee78:text-default-font line-clamp-1 shrink-0 grow basis-0">
              {children}
            </span>
          ) : null}
          {rightSlot ? (
...
const ContextMenuRoot = React.forwardRef<HTMLDivElement, ContextMenuRootProps>(
  function ContextMenuRoot(
    ...
    return children ? (
      <div
        className={SubframeUtils.twClassNames(
          "border-neutral-border bg-default-background flex min-w-[192px] flex-col items-start rounded-md border border-solid px-1 py-1 shadow-lg",
          className,
        )}
        ref={ref}
        {...otherProps}
