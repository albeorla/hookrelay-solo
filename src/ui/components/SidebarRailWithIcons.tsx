...
>(function SidebarRailWithIconsRoot(
  ...
  return (
    <nav
      className={SubframeUtils.twClassNames(
        "border-neutral-border bg-default-background flex h-full flex-col items-start border-r border-solid",
        className,
      )}
      ref={ref}
      {...otherProps}
    >
      ...
      {children ? (
        <div className="flex w-full shrink-0 grow basis-0 flex-col items-center gap-1 overflow-auto px-3 py-3">
          {children}
        </div>
      ) : null}
      {footer ? (
        <div className="border-neutral-border flex w-full flex-col items-center justify-end gap-1 border-t border-solid px-3 py-3">
          {footer}
        </div>
      ) : null}
    </nav>
