    ...
    <Accordion
      ...
      trigger={
        <div className="flex w-full items-center gap-2 rounded-md px-3 py-2 group-hover/c841484c:bg-neutral-50">
          ...
          {label ? (
            <span className="text-body font-body text-default-font line-clamp-1 shrink-0 grow basis-0">
              {label}
            </span>
          ) : null}
          <Accordion.Chevron />
  ...
  return (
    ...
    >
      ...
      {label ? (
        <span
          className={SubframeUtils.twClassNames(
            "text-body font-body text-default-font line-clamp-1 shrink-0 grow basis-0",
            { "text-brand-700": selected },
          )}
        >
          {label}
